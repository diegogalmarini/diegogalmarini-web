...
export const ClientForm: React.FC<ClientFormProps> = ({
  ...
  useEffect(() => {
    ...
    if (client) {
      console.log('🔄 Actualizando formulario con datos del cliente:', {
        id: client.id,
        name: client.name,
        email: client.email,
      ...
      console.log('🔄 No hay cliente, reseteando formulario');
      reset({
        name: '',
        email: '',
        phone: '',
  ...
  const validateEmailUnique = useCallback(async (email: string) => {
    // Solo validar si el email es diferente al email original del cliente
  ...
  const onFormSubmit = useCallback(async (data: ClientFormData) => {
    ...
    try {
      ...
      // Preparar datos - SIEMPRE incluir todos los campos (incluso si están vacíos)
      const formData: any = {
        name: data.name,
        email: data.email,
      ...
      // Si se proporciona onSubmit, delegar la responsabilidad al componente padre
      if (onSubmit) {
        await onSubmit(formData);
      } else {
        // Usar la lógica interna solo si no hay onSubmit del padre
        let result;
        if (client) {
