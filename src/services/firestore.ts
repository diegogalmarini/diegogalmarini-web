...
export const clientService = {
  ...
  async getAll(filters?: ClientFilters, pagination?: PaginationOptions): Promise<ApiResponse<PaginatedResponse<Client>>> {
    try {
      ...
      const clients: Client[] = querySnapshot.docs.map(doc => {
        ...
        // Validar que el documento tiene ID válido
        if (!doc.id || typeof doc.id !== 'string' || doc.id.trim() === '') {
          console.warn('Documento de cliente sin ID válido:', doc.id, data);
          return null;
        }

        // Validar datos mínimos requeridos
        if (!data || typeof data !== 'object') {
          console.warn('Datos de cliente inválidos para ID:', doc.id, data);
          return null;
        }

        // 🚨 LOG ANTES DE CONSTRUIR EL CLIENTE
        ...
        const client: Client = {
          ...
          language: data.language || '',  // ✅ AÑADIDO
          source: data.source || 'website',
          registrationDate: convertTimestampToString(data.registrationDate),
          lastContactDate: data.lastContactDate ? convertTimestampToString(data.lastContactDate) : undefined,
          totalConsultations: typeof data.totalConsultations === 'number' ? data.totalConsultations : 0,
        ...
        // Filtrar clientes nulos o inválidos
  ...
  async update(id: string, updates: Partial<Client>): Promise<ApiResponse<Client>> {
    try {
      ...
      console.log('🔥 Final updateData to send to Firestore:', updateData);
      ...
      console.log('🔥 Getting updated document...');
      const updatedDoc = await this.getById(id);
      console.log('🔥 Updated document result:', updatedDoc);
      return updatedDoc;
    } catch (error) {
...
export const communicationLogService = {
  ...
  // Obtener logs por consulta
  async getByConsultation(consultationId: string): Promise<ApiResponse<CommunicationLog[]>> {
    try {
      const q = query(
        collection(db, COLLECTIONS.COMMUNICATION_LOGS),
      ...
      console.error('Error getting communication logs by consultation:', error);
      return { success: false, error: 'Error al obtener los logs de comunicación de la consulta' };
    }
  },

  // Obtener todos los logs con filtros opcionales
  async getAll(filters?: { clientId?: string; consultationId?: string; type?: string }): Promise<ApiResponse<CommunicationLog[]>> {
    try {
      let q = query(collection(db, COLLECTIONS.COMMUNICATION_LOGS));
      ...
      if (filters?.consultationId) {
        q = query(q, where('consultationId', '==', filters.consultationId));
      }
...
export const followUpService = {
  ...
  async getAll(): Promise<ApiResponse<FollowUp[]>> {
    try {
      ...
      return { success: false, error: 'Error al obtener los seguimientos' };
    }
  },

  // Actualizar un seguimiento (genérico para cambios de estado, notas, etc.)
  async update(id: string, updates: Partial<FollowUp>): Promise<ApiResponse<FollowUp>> {
    try {
...
export const availabilityService = {
  ...
  async updateSlot(id: string, updates: Partial<AvailabilitySlot>): Promise<ApiResponse<AvailabilitySlot>> {
    try {
      const docRef = doc(db, COLLECTIONS.AVAILABILITY, id);
      // Remover campos que no deben actualizarse
      const updateData = { ...updates };
      ...
      // Obtener el documento actualizado (o construirlo si ya tenemos los datos)
      // Para simplificar, devolvemos los datos actualizados combinados
      // En una implementación real, podríamos hacer un getDoc para asegurar consistencia
      return { success: true, data: { id, ...updates } as AvailabilitySlot };
    } catch (error) {
      console.error('Error updating availability slot:', error);
      return { success: false, error: 'Error al actualizar el slot de disponibilidad' };
    }
  }
};
