...
export type PlanType = 'mail' | '30min' | '60min' | 'custom';
export type PaymentStatus = 'free' | 'pending' | 'paid';
export type ConsultationSource = 'website' | 'referral' | 'direct' | 'social_media';
...
export interface Consultation {
  ...
  planType: PlanType; // Tipo de plan incluyendo comunicación por mail
  ...
  source: ConsultationSource; // Fuente de la consulta
}
...
export type AppointmentType = 'consultation' | 'follow-up' | 'planning' | 'review';
...
export type CommunicationChannel = 'email' | 'call' | 'meeting' | 'note' | 'sms' | 'whatsapp';
...
export interface AppointmentFormData {
  ...
  description: string;
  clientId: string;
  type: 'consultation' | 'follow-up' | 'planning' | 'review';
  planType: 'mail' | '30min' | '60min' | 'custom';
  paymentStatus: 'free' | 'pending' | 'paid';
...
export interface Payment {
  ...
  description: string;
  dueDate: string; // ISO string - fecha límite de pago
  paidAt?: string; // ISO string - cuando se realizó el pago
  createdAt: string; // ISO string
  updatedAt: string; // ISO string
...
export interface ClientHistory {
  id: string;
  clientId: string;
  type: 'consultation' | 'appointment' | 'payment' | 'communication' | 'note';
  title: string;
  description: string;
  date: string; // ISO string
  status: string;
...
export interface AppointmentReminder {
  ...
  clientId: string;
  type: 'email' | 'sms' | 'push';
  scheduledFor: string; // ISO string - cuándo enviar el recordatorio
  sentAt?: string; // ISO string - cuándo se envió
