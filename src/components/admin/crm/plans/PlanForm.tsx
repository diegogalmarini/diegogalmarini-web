  ...
  const onFormSubmit = async (data: PlanFormData) => {
    ...
    try {
      const formattedData = {
        ...
        duration: Number(data.duration.replace(/[^0-9]/g, ''))
      };

      if (isEditing && planId) {
        // Actualizar plan existente
        updatePlan(planId, formattedData);
        setAlert({ type: 'success', message: 'Plan actualizado correctamente' });
      } else {
        // Crear nuevo plan
        addPlan(formattedData as any);
      ...
      <div className="bg-gray-50 rounded-lg p-6">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          ...
          {control._formValues.isActive !== undefined && (
            <div className="mt-2">
              <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${control._formValues.isActive
