...
export const useClients = (initialFilters?: ClientFilters) => {
  ...
  const updateClient = useCallback(async (id: string, updates: Partial<Client>) => {
    console.log('🔥 useCRM updateClient called with:', { id, updates });
    const result = await clientService.update(id, updates);
    console.log('🔥 clientService.update result:', result);

    if (result.success && result.data) {
      console.log('🔥 Reloading clients to ensure data consistency');
      // Recargar la lista completa para asegurar datos actualizados
      await loadClients();
      console.log('✅ Client updated and list reloaded successfully');
      return result.data;
    } else {
  ...
  const searchClients = useCallback(async (searchFilters: ClientFilters) => {
    try {
      const result = await clientService.getAll(searchFilters, { page: 1, limit: 10 });
...
export const useAppointments = ({ initialFilters, pagination, sort }: { initialFilters?: AppointmentFilters, pagination?: PaginationOptions, sort?: { field: keyof Appointment, direction: 'asc' | 'desc' } } = {}) => {
  ...
  const updateAppointment = useCallback(async (id: string, updates: Partial<Appointment>) => {
    ...
    if (updates.date || updates.startTime || updates.duration) {
      ...
      if (currentAppointment) {
        ...
        const time = updates.startTime || currentAppointment.startTime;
  ...
  const updateAppointmentStatus = useCallback(async (id: string, status: Appointment['status']) => {
    const result = await appointmentService.update(id, { status });
    if (result.success) {
      await loadAppointments();
      return result.data;
    } else {
      setError(result.error || 'Error al actualizar estado de la cita');
      return null;
    }
  }, [loadAppointments]);
...
export const useAvailability = (date?: Date) => {
  ...
  // Memoize date string to avoid re-renders if date object changes but represents same day
  // If no date provided, use current date but keep it stable for the dependency array
  ...
  const loadAvailability = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await availabilityService.getByDate(dateStr);
      if (result.success) {
        setAvailabilitySlots(result.data!);
      } else {
        setError(result.error || 'Error al cargar la disponibilidad');
      }
    } catch (err) {
      setError('Error inesperado al cargar la disponibilidad');
    } finally {
      setLoading(false);
    }
  }, [dateStr]);
  ...
  const updateAvailabilitySlot = useCallback(async (id: string, updates: Partial<AvailabilitySlot>) => {
    const result = await availabilityService.updateSlot(id, updates);

    if (result.success) {
      setAvailabilitySlots(prev => prev.map(slot => slot.id === id ? { ...slot, ...updates } : slot));
      return result.data;
    } else {
      setError(result.error || 'Error al actualizar slot de disponibilidad');
      return null;
    }
  }, []);
  ...
  useEffect(() => {
    loadAvailability();
    loadBlockedPeriods();
  }, [loadAvailability, loadBlockedPeriods]);
...
export const useFollowUps = () => {
  ...
  const loadFollowUps = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || 'Error al cargar todos los seguimientos');
      }
    } catch (err) {
      setError('Error inesperado al cargar los seguimientos');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  // Actualizar seguimiento (para Kanban u otros cambios de estado/notas)
  const updateFollowUp = useCallback(async (id: string, updates: Partial<FollowUp>) => {
    const result = await followUpService.update(id, updates);
    ...
    if (result.success) {
      ...
      setError(result.error || 'Error al actualizar el seguimiento');
      return null;
    }
  }, [loadFollowUps]);
...
export const useCommunicationLogs = (clientId?: string, consultationId?: string) => {
  ...
  const loadLogs = useCallback(async () => {
    ...
    try {
      ...
      if (consultationId) {
        result = await communicationLogService.getByConsultation(consultationId);
      } else if (clientId) {
        result = await communicationLogService.getByClient(clientId);
      } else {
        result = await communicationLogService.getAll();
      }
  ...
  }, [loadLogs, clientId, consultationId]);
