...
export const PlanTypeBadge: React.FC<{ planType?: string; className?: string }> = ({ planType, className }) => {
  const getVariant = (planType?: string) => {
    ...
    switch (planType.toLowerCase()) {
      case 'mail':
        return 'success'; // Verde para consultas gratuitas
      case '30min':
      case 'consultation_30':
        return 'info'; // Azul para consultas de 30 min
      case '60min':
      case 'consultation_60':
        return 'secondary'; // Púrpura para consultas de 60 min
      case 'custom':
        return 'warning'; // Amarillo para consultas personalizadas
      case 'follow_up':
        return 'warning';
